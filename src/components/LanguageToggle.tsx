...
export const LanguageToggle: React.FC<LanguageToggleProps> = ({ 
  ...
  if (variant === 'topbar') {
    return (
      <div className={`inline-flex items-center rounded-full bg-slate-900/90 p-0.5 border border-slate-800 text-xs shadow-xs ${className}`}>
        <Globe className="w-3 h-3 text-maroon-400 ml-1.5 mr-0.5" />
        <button
          type="button"
          onClick={() => setLanguage('en')}
          className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold transition-all cursor-pointer ${
            language === 'en'
              ? 'bg-maroon-800 text-white shadow-xs border border-maroon-500/50'
  ...
  if (variant === 'footer') {
    return (
      <div className={`inline-flex items-center gap-2 ${className}`}>
        <Globe className="w-3.5 h-3.5 text-maroon-800" />
        <div className="inline-flex items-center rounded-lg bg-white p-0.5 border border-rose-200 text-xs shadow-2xs">
          <button
            type="button"
            onClick={() => setLanguage('en')}
            className={`px-2.5 py-1 rounded-md font-bold transition-all cursor-pointer ${
              language === 'en'
                ...
                : 'text-slate-600 hover:text-maroon-800'
            }`}
          >
            English
          </button>
